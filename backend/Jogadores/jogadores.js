...
const criarJogador = (req, res) => {
  const { nome_completo, numero, usuario_id, time_id, idade, posicao } = req.body;
  ...
  pool.execute(query, parametros, (err, results) => {
    if (err) {
      return res.status(500).json({ message: 'Erro ao criar o jogador', error: err.message });
    }
...
const editarJogador = (req, res) => {
  ...
  pool.execute(querySelect, [id], (err, results) => {
    if (err) {
      return res.status(500).json({ message: 'Erro ao buscar o jogador', error: err.message });
    }
    ...
    const novoUsuarioId = usuario_id !== undefined ? usuario_id : jogador.usuario_id;
    ...
    const queryUpdate = `
      ...
      WHERE id = ?
    `;
    pool.execute(queryUpdate, [novoNome, novoNumero, novoUsuarioId, novoTimeId, novaIdade, novaPosicao, id], (err, results) => {
      if (err) {
        return res.status(500).json({ message: 'Erro ao atualizar o jogador', error: err.message });
      }
      res.status(200).json({ message: 'Jogador atualizado com sucesso', result: results });
    });
  });
};

module.exports = { listarJogadores, criarJogador, deletarJogador, editarJogador };
